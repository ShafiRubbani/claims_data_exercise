...
regression_variables <- individual_annual %>% 
  ...
  mutate(minority = ifelse(is.na(race), NA, if_else(race == "w", 0, 1))) %>% 
  # mutate(minority = if_else(race == "w", 0, 1)) %>% 
  select(total, lower_SES, age, female, minority, months_enrolled)
```
...
Mean: `r regression_variables %>% summarize(x = mean(minority))`, 
Standard Deviation: `r regression_variables %>% summarize(x = sd(minority))`, 
Median: `r regression_variables %>% summarize(x = median(minority))`, 
Min: `r regression_variables %>% summarize(x = min(minority))`, 
Max: `r regression_variables %>% summarize(x = max(minority))`
...
SES_vs_annual_costs <- lm(total ~ lower_SES + age + female + minority + months_enrolled,
